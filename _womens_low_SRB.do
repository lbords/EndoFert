...
// First birth controls
	reghdfe w_ideal_kids_trunc recent_firstborn_daughter recent_firstborn [aweight=wght_avg_pop] if low_SRB == 1, ///
	absorb(i.std_adm_region_code##i.interview_month i.interview_year) ///
	vce(cluster i.std_adm_region_code)

	outreg2 using "raid/lfborden/Endogenous_Fertility/reg_output/womens_low_SRB.tex", ///
...
// First birth controls + demographic controls
	...
	edu_yrs_trunc rural total_children poor [aweight=wght_avg_pop] if low_SRB == 1, ///
	absorb(i.std_adm_region_code##i.interview_month i.interview_year) ///
	vce(cluster i.std_adm_region_code)
...
// First birth controls
	reghdfe w_ideal_girls_trunc recent_firstborn_daughter recent_firstborn [aweight=wght_avg_pop] if low_SRB == 1, ///
	absorb(i.std_adm_region_code##i.interview_month i.interview_year) ///
	vce(cluster i.std_adm_region_code)
...
// First birth controls
	reghdfe w_ideal_boys_trunc recent_firstborn_daughter recent_firstborn [aweight=wght_avg_pop] if low_SRB == 1, ///
	absorb(i.std_adm_region_code##i.interview_month i.interview_year) ///
	vce(cluster i.std_adm_region_code)
